...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .error {
            background: #fee; color: #c33; padding: 12px 16px;
            border-radius: 12px; margin-bottom: 1rem; font-size: 0.9rem;
            border-left: 4px solid #c33;
        }
        .success {
            background: #efe; color: #3c3; padding: 12px 16px;
            border-radius: 12px; margin-bottom: 1rem; font-size: 0.9rem;
        ...
        .tab {
            ...
            transition: all 0.2s; font-weight: 500; color: #666;
        }
        .tab.active {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            color: white;
...
<body>
    <div class="login-card">
        ...
        <div class="tab-content active" id="login-form">
            <div class="form-group">
                <label>账号 / UID</label>
                <input type="text" id="login-identifier" placeholder="输入账号或UID" autofocus>
            </div>
            <div class="form-group">
                <label>密码</label>
        ...
        <div class="tab-content" id="register-form">
            ...
            <div class="form-group">
                <label>显示名称</label>
                <input type="text" id="reg-display-name" placeholder="设置显示名称">
            </div>
            <div class="form-group">
                <label>密码</label>
    ...
    <script>
        ...
        document.getElementById('btn-login').addEventListener('click', async function() {
            ...
            if (!id || !pwd) { showMsg('请填写所有字段', 'error'); return; }
            ...
            try {
                ...
                if (d.success) {
                    ...
                    localStorage.setItem('web_uid', d.uid);
                    localStorage.setItem('web_name', d.name);
        ...
        document.getElementById('btn-register').addEventListener('click', async function() {
            ...
            if (!u || !p) { showMsg('请填写所有字段', 'error'); return; }
            const btn = document.getElementById('btn-register');
            btn.disabled = true; btn.textContent = '注册中...';
            try {
                const r = await fetch('/web/api/register', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
        ...
        const saved = localStorage.getItem('web_uid');
        if (saved) document.getElementById('login-identifier').value = saved;
    </script>
</body>
</html>
"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        :root {
            --primary: #667eea; --bg: #f0f2f5; --sidebar-bg: #ffffff;
            --chat-bg: #f8f9fa; --msg-own: #667eea; --msg-other: #ffffff;
            --text: #333; --text-muted: #888; --border: #e0e0e0;
        }
        html, body { height: 100%; font-family: 'Segoe UI', Roboto, sans-serif; background: var(--bg); }
        ...
        .user-info { flex: 1; }
        .user-info strong { font-size: 1rem; color: var(--text); }
        ...
        .contact-item.active { background: #e8ecff; }
        .contact-avatar {
        ...
        .message.own {
            background: var(--msg-own); color: white; align-self: flex-end;
            border-bottom-right-radius: 4px;
        }
        .message-sender {
            font-size: 0.75rem; color: var(--primary); margin-bottom: 4px; font-weight: 500;
        }
        .message-time {
            font-size: 0.65rem; color: var(--text-muted); margin-top: 4px; text-align: right;
        }
        ...
        .empty-chat i { font-size: 4rem; margin-bottom: 16px; opacity: 0.3; }
        .empty-chat p { font-size: 1.1rem; }
        .online-dot {
...
<body>
    ...
    <script>
        const state = {
            ...
            ws: null, currentChat: null, contacts: [], messages: {}
        };
        function esc(s) {
        ...
        function connectWS() {
            ...
            state.ws = new WebSocket(p + '//' + location.host + '/web/ws?token=' + encodeURIComponent(state.token));
            state.ws.onopen = function() {
                console.log('WS connected');
                state.ws.send(JSON.stringify({type: 'auth', token: state.token, uid: state.uid}));
            };
            ...
            state.ws.onerror = function(e) { console.error('WS error', e); };
        }
        function handleMsg(data) {
            switch(data.type) {
                case 'auth_ok':
                    document.getElementById('my-name').textContent = data.name || state.name;
                    loadContacts(); break;
                case 'contacts': renderContacts(data.list); break;
                case 'messages': renderMessages(data); break;
                case 'new_message': addMessage(data); break;
                case 'error': alert(data.msg || '错误'); break;
            }
        }
        ...
        function renderContacts(list) {
            ...
            if (!list || list.length === 0) {
                el.innerHTML = '<div style="padding: 20px; text-align: center; color: #888;">暂无好友</div>';
                return;
            }
            el.innerHTML = list.map(function(c) {
                return '<div class="contact-item" data-uid="' + c.uid + '">' +
                    '<div class="contact-avatar">' +
                    (c.avatar ? '<img src="' + c.avatar + '" alt="">' : (c.name||c.uid).charAt(0).toUpperCase()) +
                    '</div><div class="contact-info"><div class="contact-name">' +
                    esc(c.name || c.uid) +
                    ' <span class="' + (c.online ? 'online-dot' : 'offline-dot') + '"></span>' +
                    '</div><div class="contact-preview">' + esc(c.last || '') + '</div></div></div>';
            }).join('');
            el.querySelectorAll('.contact-item').forEach(function(item) {
                item.addEventListener('click', function() { selectChat(item); });
            });
        }
        function selectChat(el) {
            ...
            const c = state.contacts.find(function(x) { return x.uid === uid; });
            state.currentChat = {uid: uid, name: c ? c.name : uid};
            document.getElementById('chat-title').textContent = c ? c.name : uid;
            document.getElementById('empty-state').style.display = 'none';
            document.getElementById('chat-messages').style.display = 'flex';
            if (state.ws && state.ws.readyState === 1)
                state.ws.send(JSON.stringify({type: 'get_messages', uid: uid}));
        }
        function renderMessages(data) {
            const box = document.getElementById('chat-messages');
            state.messages[data.uid] = data.list || [];
            box.innerHTML = (data.list || []).map(function(m) {
                return '<div class="message ' + (m.from_me ? 'own' : 'other') + '">' +
                    (!m.from_me ? '<div class="message-sender">' + esc(m.sender) + '</div>' : '') +
                    '<div class="message-content">' + esc(m.body) + '</div>' +
                    '<div class="message-time">' + fmtTime(m.time) + '</div></div>';
            }).join('');
            box.scrollTop = box.scrollHeight;
        }
        function addMessage(data) {
            if (!state.messages[data.uid]) state.messages[data.uid] = [];
            state.messages[data.uid].push(data);
            if (state.currentChat && state.currentChat.uid === data.uid) {
                const box = document.getElementById('chat-messages');
                const own = data.from_me;
                const html = '<div class="message ' + (own ? 'own' : 'other') + '">' +
                    (!own ? '<div class="message-sender">' + esc(data.sender) + '</div>' : '') +
                    '<div class="message-content">' + esc(data.body) + '</div>' +
                    '<div class="message-time">' + fmtTime(data.time) + '</div></div>';
        ...
        function sendMsg() {
            ...
            if (!txt || !state.currentChat || !state.ws) return;
            state.ws.send(JSON.stringify({
                type: 'send_message', uid: state.currentChat.uid, body: txt
            }));
        ...
        document.getElementById('btn-logout').addEventListener('click', function() {
            localStorage.clear(); window.location.href = '/web/login';
        });
...
def web_api_login():
    ...
    now = now_ts()
    db_execute(
        "INSERT INTO tokens (user_id, access_token, created_at, expires_at) VALUES (?, ?, ?, ?)",
...
def web_api_register():
    ...
    user_row = db_query_one("SELECT * FROM users WHERE id = ?", (uid_int,))
    # 自动加入默认群
    default_gid = _ensure_default_group()
    if default_gid and user_row:
        try:
            latest_before = db_query_one(
                "SELECT MAX(id) as max_id FROM group_messages WHERE group_id = ?",
                (default_gid,),
            ...
            log.warning("[web register] auto-join default group failed: %s", _e)
    token = secrets.token_hex(32)
    db_execute(
        "INSERT INTO tokens (user_id, access_token, created_at, expires_at) VALUES (?, ?, ?, ?)",
        (uid_int, token, now, now + 86400 * 30),
    )
    return jsonify({"success": True, "token": token, "uid": uid})

# Web WebSocket 连接
_web_ws_clients = {}  # sid -> {user_id, uid}

def _web_ws_get_user(token):
    if not token:
        return None
    trow = db_query_one("SELECT user_id, expires_at FROM tokens WHERE access_token = ?", (token,))
    if not trow:
        return None
    if trow["expires_at"] and trow["expires_at"] < now_ts():
        return None
    urow = db_query_one("SELECT id, uid, display_name, username, avatar_url FROM users WHERE id = ?", (trow["user_id"],))
    return urow

def web_ws_handler(environ, start_response):
    ...
    def _handler(ws):
        ...
        try:
            while True:
                ...
                if mtype == "auth":
                    token = msg.get("token")
                    urow = _web_ws_get_user(token)
                    if urow:
                        auth_ok = True
                        user_id = urow["id"]
                        uid = urow["uid"]
                        _web_ws_clients[sid] = {"ws": ws, "user_id": user_id, "uid": uid}
                        ws.send(json.dumps({
                            "type": "auth_ok",
                            "uid": uid,
                            "name": urow["display_name"] or urow["username"],
                        }))
                    else:
                        ws.send(json.dumps({"type": "error", "msg": "认证失败"}))
                        return
                elif auth_ok and mtype == "get_contacts":
                    rows = db_query_all(
                        """SELECT u.* FROM users u WHERE u.id IN (
                            SELECT CASE WHEN f.user_id = ? THEN f.friend_id ELSE f.user_id END
                            FROM friendships f
                    ...
                    online_ids = set(ws_online_user_ids())
                    contacts = []
                    for r in rows:
                        last_msg = ""
                        mr = db_query_one(
                    ...
                    ws.send(json.dumps({"type": "contacts", "list": contacts}))
                elif auth_ok and mtype == "get_messages":
                    target_uid = msg.get("uid", "")
                    target = db_query_one("SELECT id, uid, display_name FROM users WHERE uid = ?", (target_uid.upper(),))
                    if not target:
                        ws.send(json.dumps({"type": "messages", "uid": target_uid, "list": []}))
                        continue
                    target_id = target["id"]
                    mrows = db_query_all(
                        """SELECT dm.* FROM direct_messages dm
                           WHERE (dm.from_id = ? AND dm.to_id = ?) OR (dm.from_id = ? AND dm.to_id = ?)
                           ORDER BY dm.id ASC LIMIT 200""",
                        (user_id, target_id, target_id, user_id),
                    )
                    msgs = []
                    for m in mrows:
                        sender_name = "我" if m["from_id"] == user_id else (target["display_name"] or target["uid"])
                        msgs.append({
                            "id": m["id"],
                            "from_me": m["from_id"] == user_id,
                            "sender": sender_name,
                            "body": m["body"] or "",
                            "time": m["created_at"],
                        })
                    ws.send(json.dumps({"type": "messages", "uid": target_uid, "list": msgs}))
                elif auth_ok and mtype == "send_message":
                    target_uid = msg.get("uid", "")
                    body = msg.get("body", "")
                    if not target_uid or not body:
                        continue
                    target = db_query_one("SELECT id, uid, display_name FROM users WHERE uid = ?", (target_uid.upper(),))
                    if not target:
                        ws.send(json.dumps({"type": "error", "msg": "用户不存在"}))
                        continue
                    target_id = target["id"]
                    now = now_ts()
                    mid = db_execute(
                        "INSERT INTO direct_messages (from_id, to_id, body, created_at, is_read) VALUES (?, ?, ?, ?, 0)",
                        (user_id, target_id, body, now),
                    )
                    msg_obj = {
                        "id": mid,
                        "from_uid": uid,
                        "from_name": db_query_one("SELECT display_name FROM users WHERE id = ?", (user_id,))["display_name"] or uid,
                        "from_avatar": "",
                        "to_uid": target["uid"],
                        "body": body,
                        "created_at": now,
                    }
                    push_to_user(target_id, {"type": "direct_message", "data": msg_obj})
                    my_name = db_query_one("SELECT display_name FROM users WHERE id = ?", (user_id,))
                    ws.send(json.dumps({
                        "type": "new_message",
                        "uid": target_uid,
                        "id": mid,
                        "from_me": True,
                        "sender": my_name["display_name"] or uid if my_name else "我",
                        "body": body,
                        "time": now,
                    }))
        except Exception as e:
            log.info("web ws error: %s", e)
        finally:
            if sid in _web_ws_clients:
                del _web_ws_clients[sid]
    return _handler(environ, start_response)

# 更新 dispatch 函数，加入 /web/ws 路由
if HAS_EVENTLET:
    _orig_dispatch = dispatch
    def _new_dispatch(environ, start_response):
        path = environ.get("PATH_INFO", "")
        if path == "/v1/ws":
            return ws_handler(environ, start_response)
        if path == "/web/ws":
            return web_ws_handler(environ, start_response)
        return app(environ, start_response)
...
if __name__ == "__main__":
    ...
    print(" Web 界面: http://%s:%d/web/" % (host, port))
    if IS_WINDOWS:
