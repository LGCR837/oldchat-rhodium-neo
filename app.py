...
def web_index():
    ...
    user = db_query_one("SELECT * FROM users WHERE uid = ?", (uid,))
    if not user:
        session.clear()
        return redirect("/web/login")
...
def web_api_register():
    ...
    if default_gid:
        try:
            ...
            log.warning("[web register] auto-join failed: %s", _e)
    session["web_uid"] = uid
...
def _web_current_user():
    ...
    if not uid:
        return None
    return db_query_one("SELECT * FROM users WHERE uid = ?", (uid,))

@app.route("/web/api/contacts")
...
@app.route("/web/api/messages/<msg_type>/<target_id>")
def web_api_messages(msg_type, target_id):
    ...
    offset = int(request.args.get("offset", 0))
    if msg_type == "direct":
        target = db_query_one("SELECT id, uid, display_name, avatar_url FROM users WHERE uid = ?", (target_id.upper(),))
        if not target:
...
@app.route("/web/api/mark_read/<msg_type>/<target_id>", methods=["PUT"])
def web_api_mark_read(msg_type, target_id):
    ...
    if msg_type == "direct":
        ...
        group = db_query_one("SELECT id, group_id FROM groups WHERE group_id = ?", (target_id,))
        if group:
            latest = db_query_one(
...
@app.route("/web/api/send", methods=["POST"])
def web_api_send():
    user = _web_current_user()
    if not user:
        return jsonify({"error": "未登录"}), 401
...
@app.route("/web/api/themes")
def web_api_themes():
    return jsonify([{"id": "momopink", "name": "Momo Pink", "preview": ""}])

@app.route("/web/api/set_theme", methods=["POST"])
def web_api_set_theme():
    return jsonify({"success": True})

@app.route("/web/api/upload_and_send", methods=["POST"])
...
@app.route("/web/api/emoticons")
def web_api_emoticons():
    return jsonify([])

@app.route("/web/api/emoji/plaza")
