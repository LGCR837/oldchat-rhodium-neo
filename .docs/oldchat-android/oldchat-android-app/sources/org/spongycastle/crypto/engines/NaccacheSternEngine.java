    ...
    public byte[] processBlock(byte[] bArr, int i2, int i3) throws InvalidCipherTextException {
        ...
        for (int i4 = 0; i4 < smallPrimes.size(); i4++) {
            ...
            if (iIndexOf == -1) {
                if (this.debug) {
                    PrintStream printStream = System.out;
                    printStream.println("Actual prime is " + smallPrimes.elementAt(i4));
                    printStream.println("Decrypted value is " + bigIntegerModPow);
                    printStream.println("LookupList for " + smallPrimes.elementAt(i4) + " with size " + this.lookup[i4].size() + " is: ");
