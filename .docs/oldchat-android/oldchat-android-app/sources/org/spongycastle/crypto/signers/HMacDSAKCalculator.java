...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private final byte[] f7435K;

    /* JADX INFO: renamed from: V, reason: collision with root package name */
    private final byte[] f7436V;
    private final HMac hMac;

    /* JADX INFO: renamed from: n, reason: collision with root package name */
    private BigInteger f7437n;

    public HMacDSAKCalculator(Digest digest) {
        HMac hMac = new HMac(digest);
        this.hMac = hMac;
        this.f7436V = new byte[hMac.getMacSize()];
        this.f7435K = new byte[hMac.getMacSize()];
    }

    private BigInteger bitsToInt(byte[] bArr) {
        BigInteger bigInteger = new BigInteger(1, bArr);
        return bArr.length * 8 > this.f7437n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f7437n.bitLength()) : bigInteger;
    }
    ...
    public BigInteger nextK() {
        int iBitLength = (this.f7437n.bitLength() + 7) / 8;
        byte[] bArr = new byte[iBitLength];
        while (true) {
            int i2 = 0;
            while (i2 < iBitLength) {
                HMac hMac = this.hMac;
                byte[] bArr2 = this.f7436V;
                hMac.update(bArr2, 0, bArr2.length);
                this.hMac.doFinal(this.f7436V, 0);
                int iMin = Math.min(iBitLength - i2, this.f7436V.length);
                System.arraycopy(this.f7436V, 0, bArr, i2, iMin);
                i2 += iMin;
            }
            BigInteger bigIntegerBitsToInt = bitsToInt(bArr);
            if (bigIntegerBitsToInt.compareTo(ZERO) > 0 && bigIntegerBitsToInt.compareTo(this.f7437n) < 0) {
                return bigIntegerBitsToInt;
            }
            HMac hMac2 = this.hMac;
            byte[] bArr3 = this.f7436V;
            hMac2.update(bArr3, 0, bArr3.length);
            this.hMac.update((byte) 0);
            this.hMac.doFinal(this.f7435K, 0);
            this.hMac.init(new KeyParameter(this.f7435K));
            HMac hMac3 = this.hMac;
            byte[] bArr4 = this.f7436V;
    ...
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f7437n = bigInteger;
        Arrays.fill(this.f7436V, (byte) 1);
        Arrays.fill(this.f7435K, (byte) 0);
        ...
        this.hMac.init(new KeyParameter(this.f7435K));
        HMac hMac2 = this.hMac;
        byte[] bArr5 = this.f7436V;
        hMac2.update(bArr5, 0, bArr5.length);
        this.hMac.doFinal(this.f7436V, 0);
        HMac hMac3 = this.hMac;
        byte[] bArr6 = this.f7436V;
